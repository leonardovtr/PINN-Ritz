...
import square_basis_selection_core as base  # noqa: E402
...
def main() -> None:
    ...
    for k in range(args.seeds):
        ...
        pd.DataFrame(all_mode_rows).to_csv(OUT_DATA / "square_basis_selection_modes.csv", index=False)
        pd.DataFrame(seed_rows).to_csv(OUT_DATA / "square_basis_selection_summary.csv", index=False)

    df_modes = pd.DataFrame(all_mode_rows)
    df_seed = pd.DataFrame(seed_rows)
    modes_path = OUT_DATA / "square_basis_selection_modes.csv"
    seed_path = OUT_DATA / "square_basis_selection_summary.csv"
